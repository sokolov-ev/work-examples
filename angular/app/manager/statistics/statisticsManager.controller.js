(function () {
    ...
    angular
        .module('app')
        .controller('StatisticsManagerController', StatisticsManagerController);

    StatisticsManagerController.$inject = ['Page', 'Statistics', '$state', 'localStorageService', 'Auth', '$stateParams'];

    function StatisticsManagerController(Page, Statistics, $state, localStorageService, Auth, $stateParams) {
        let vm = this;
        ...
        function init () {
            ...
            if (vm.dates.format == 'month') {
                $state.go('manager.general-statistics', {format: 'month', day: moment(new Date(vm.dates.start)).format('YYYY-MM-DD')}, {notify: false});
            } else {
                $state.go('manager.general-statistics', {format: null, day: null}, {notify: false});
            }
            ...
            .datepicker("setDate", new Date(vm.dates.start))
            .on("changeDate", function(event) {
                $state.go('manager.general-statistics', {format: 'month', day: moment(new Date(event.date)).format('YYYY-MM-DD')}, {notify: false});
        ...
        function goToStat (user, day) {
            if (day !== 'sum') {
                $state.go('manager.developer-work-diary', {id: user, day: day});
            }
        };
